...
- [Development guidelines](#development-guidelines)
    - [General STOMP Core](#general-stomp-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
STOMP Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play STMPs (tSTMP)" on the test network, if you
...
STOMP Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/stomp-project/stomp/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_stomp
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_stomp --log_level=test_suite
$ valgrind -v --leak-check=full src/stompd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_stomp.coverage/index.html`.
```
...
- BitcoinMiner : Generates STMPs (if wallet is enabled).
...
However, in open source software such as STOMP Core, where everyone uses
...
pay attention to for reviewers of STOMP Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: STOMP Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against STOMP Core so that they can be integrated
...
file descriptors it will cause problems with STOMP's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:STOMP-Project/STOMP.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `stomp-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `stomp-cli` and the GUI debug console use this table to determine how to
