...
* Update translations (ping Fuzzbawls on Discord) see [translation_process.md](https://github.com/STOMP-Project/STOMP/blob/master/doc/translation_process.md#synchronising-translations).
* Update manpages, see [gen-manpages.sh](https://github.com/stomp-project/stomp/blob/master/contrib/devtools/README.md#gen-manpagessh).
...
By default, Gitian will fetch source files as needed. To cache them ahead of time, make sure you have checked out the tag you want to build in stomp, then:
...
### Build and sign STOMP Core for Linux, Windows, and macOS:

    pushd ./gitian-builder
    ./bin/gbuild --num-make 2 --memory 3000 --commit stomp=v${VERSION} ../stomp/contrib/gitian-descriptors/gitian-linux.yml
    ...
    ./bin/gbuild --num-make 2 --memory 3000 --commit stomp=v${VERSION} ../stomp/contrib/gitian-descriptors/gitian-win.yml
    ...
    ./bin/gbuild --num-make 2 --memory 3000 --commit stomp=v${VERSION} ../stomp/contrib/gitian-descriptors/gitian-osx.yml
...
Build output expected:
  ...
  4. macOS unsigned installer and dist tarball (`stomp-${VERSION}-osx-unsigned.dmg`, `stomp-${VERSION}-osx64.tar.gz`)
...
Codesigner only: Sign the macOS binary:
    ...
    transfer stomp-osx-unsigned.tar.gz to macOS for signing
...
stomp-${VERSION}-riscv64-linux-gnu.tar.gz
