...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zstmpspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 STMP at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zSTMP send
enum ZerocoinSpendStatus {
    ZSTMP_SPEND_OKAY = 0,                            // No error
    ZSTMP_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZSTMP_WALLET_LOCKED = 2,                         // Wallet was locked
    ZSTMP_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZSTMP_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZSTMP_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZSTMP_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZSTMP_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZSTMP_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZSTMP_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZSTMP_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZSTMP_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZSTMP_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZSTMP_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZSTMP_SPENT_USED_ZSTMP = 14,                      // Coin has already been spend
    ZSTMP_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZSTMP_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZStmpBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZSTMPOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzstmpAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzSTMPTracker> zstmpTracker;
    ...
    void setZWallet(CzSTMPWallet* zwallet)
    {
        zwalletMain = zwallet;
        zstmpTracker = std::unique_ptr<CzSTMPTracker>(new CzSTMPTracker(strWalletFile));
    }

    CzSTMPWallet* getZWallet() { return zwalletMain; }
    ...
    void setZStmpAutoBackups(bool fEnabled)
    {
    ...
    /** zSTMP reset */
    boost::signals2::signal<void()> NotifyzSTMPReset;
