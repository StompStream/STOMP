...
public:
    ...
    bool WriteZSTMPSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZSTMPSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZSTMPSeed_deprecated(uint256& seed);
    bool EraseZSTMPSeed();
    bool EraseZSTMPSeed_deprecated();

    bool WriteZSTMPCount(const uint32_t& nCount);
    bool ReadZSTMPCount(uint32_t& nCount);
