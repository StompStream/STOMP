...
bool ContextualCheckZerocoinSpendNoSerialCheck(const CTransaction& tx, const libzerocoin::CoinSpend* spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (pindex->nHeight >= Params().Zerocoin_Block_Public_Spend_Enabled()) {
        ...
        if (v1Serial) {
            return error("%s : zSTMP v1 serial spend not spendable, serial %s, tx %s\n", __func__,
...
bool CheckZerocoinSpend(const CTransaction& tx, bool fVerifySignature, CValidationState& state, bool fFakeSerialAttack)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        ...
        if (isPublicSpend) {
            ...
            if (!ZSTMPModule::parseCoinSpend(txin, tx, prevOut, publicSpend)){
        ...
        if (isPublicSpend) {
            ...
            if (!ZSTMPModule::validateInput(txin, prevOut, tx, ret)){
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    libzerocoin::ZerocoinParams* params = Params().Zerocoin_Params(false);
                    PublicCoinSpend publicSpend(params);
                    if (!ZSTMPModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
            ...
            // Check that zSTMP mints (if included) are not already known
...
int64_t GetDevelopersPayment(int nHeight, int64_t blockValue, bool isZSTMPStake) {
    ...
    return ret;
        ...
        //When zSTMP is staked, masternode only gets 2 STMP
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZSTMPModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //    RecalculateSTMPSupply(Params().Zerocoin_StartHeight());
    //}
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Stomp Specific reference to the block with the wrong threshold was used.
        if ((block.nTime == (uint32_t) Params().StompBadBlockTime()) && (block.nBits == (uint32_t) Params().StompBadBlocknBits())) {
            // accept STOMP block minted with incorrect proof of work threshold
            return true;
        }
        ...
        if (isBlockFromFork) {
            ...
            if(hasZSTMPInputs){
                for (const CTxIn& zStmpInput : zSTMPInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zStmpInput);
