...
CzSTMPTracker::~CzSTMPTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSTMPTracker::Init()
{
...
bool CzSTMPTracker::Archive(CMintMeta& meta)
{
...
bool CzSTMPTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSTMPTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSTMPTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSTMPTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSTMPTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSTMPTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
CAmount CzSTMPTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSTMPTracker::GetUnconfirmedBalance() const
{
...
bool CzSTMPTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSTMPTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSTMPTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSTMPTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSTMPTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSTMPTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSTMPTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSTMPTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSTMPWallet* zSTMPWallet)
{
    bool iszSTMPWalletInitialized = (NULL != zSTMPWallet);
    CMintMeta meta;
    ...
    if (! iszSTMPWalletInitialized)
        zSTMPWallet = new CzSTMPWallet(strWalletFile);
    meta.isSeedCorrect = zSTMPWallet->CheckSeed(dMint);
    if (! iszSTMPWalletInitialized)
        delete zSTMPWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSTMPTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSTMPTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSTMPTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSTMPTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSTMPTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSTMPTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    ...
    if (fUpdateStatus) {
        ...
        delete zSTMPWallet;
        LogPrint("zero", "%s: added %d dzstmp from DB\n", __func__, listDeterministicDB.size());
    }
