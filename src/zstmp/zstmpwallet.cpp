...
CzSTMPWallet::CzSTMPWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zstmp seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSTMPSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSTMPSeed_deprecated()) {
                    LogPrintf("%s: Updated zSTMP seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zstmp seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zstmp wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSTMPWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSTMPCount(nCountLastUsed);
    else if (!walletdb.ReadZSTMPCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSTMPWallet::Lock()
{
    seedMaster = 0;
}

void CzSTMPWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSTMPWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSTMP(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSTMPWallet::LoadMintPoolFromDB()
{
...
void CzSTMPWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSTMPWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSTMPWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zstmpTracker->HasPubcoinHash(pMint.first)) {
...
bool CzSTMPWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSTMP(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zstmpTracker which also adds to database
    pwalletMain->zstmpTracker->Add(dMint, true);
...
void CzSTMPWallet::SeedToZSTMP(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSTMPWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSTMPWallet::GenerateDeterministicZSTMP(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSTMPWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSTMPWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSTMPWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
