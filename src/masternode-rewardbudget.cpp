...
bool CRewardBudget::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    CAmount budgetPayment = GetRewardsPayment(nBlockHeight, blockreward, txNew.HasZerocoinSpendInputs());
    ...
    if (!fundValid) {
	error("%s: invalid reward fund payment detected, expected %s, payed %s, tx:%s height:%d\n",
