...
int nAnonymizeStompAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "stomp" is a composite category enabling all STOMP-related debug output
            if (ptrCategory->count(string("stomp"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\STOMP
// Windows >= Vista: C:\Users\Username\AppData\Roaming\STOMP
// Mac: ~/Library/Application Support/STOMP
// Unix: ~/.stomp
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "STOMP";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "stomp.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override stomp.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "stompd.pid"));
