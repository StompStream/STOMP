...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total stomp balance of the wallet (excluding zerocoins)\n"
           ...
           //"     \"10\" : n,           (numeric) supply of 10 zSTMP denomination\n"
           // "     \"50\" : n,           (numeric) supply of 50 zSTMP denomination\n"
           // "     \"100\" : n,          (numeric) supply of 100 zSTMP denomination\n"
           // "     \"500\" : n,          (numeric) supply of 500 zSTMP denomination\n"
           ...
           // "  }\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in stomp/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in stomp/kb\n"
    ...
    UniValue zstmpObj(UniValue::VOBJ);
   // for (auto denom : libzerocoin::zerocoinDenomList) {
   //     zstmpObj.push_back(Pair(std::to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
   // }
   // zstmpObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    //obj.push_back(Pair("zSTMPsupply", zstmpObj));
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw std::runtime_error(
            "validateaddress \"stompaddress\"\n"
            "\nReturn information about the given stomp address.\n"

            "\nArguments:\n"
            "1. \"stompaddress\"     (string, required) The stomp address to validate\n"

            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"stompaddress\", (string) The stomp address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: STOMP address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
        throw std::runtime_error(
            ...
            "2. \"keys\"       (string, required) A json array of keys which are stomp addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) stomp address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw std::runtime_error(
            "verifymessage \"stompaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"

            "\nArguments:\n"
            "1. \"stompaddress\"  (string, required) The stomp address to use for the signature.\n"
