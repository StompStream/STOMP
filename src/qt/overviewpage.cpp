...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::STMP)
    {
    }
...
void OverviewPage::getPercentage(CAmount nUnlockedBalance, CAmount nZerocoinBalance, QString& sSTMPPercentage, QString& szSTMPPercentage)
{
    int nPrecision = 2;
    ...
    szSTMPPercentage = "(" + QLocale(QLocale::system()).toString(dzPercentage, 'f', nPrecision) + " %)";
    sSTMPPercentage = "(" + QLocale(QLocale::system()).toString(dPercentage, 'f', nPrecision) + " %)";
                              ...
                              const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    CAmount stmpAvailableBalance = balance - immatureBalance - nLockedBalance;
    CAmount nUnlockedBalance = nTotalBalance - nLockedBalance;

    // STMP Watch-Only Balance
    ...
    // Combined balances
    CAmount availableTotalBalance = stmpAvailableBalance + matureZerocoinBalance;
    CAmount sumTotalBalance = nTotalBalance + zerocoinBalance;

    // STMP labels
    ui->labelBalance->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, stmpAvailableBalance, false, BitcoinUnits::separatorAlways));
    ...
    // Percentage labels
    ui->labelSTMPPercent->setText(sPercentage);
    ui->labelzSTMPPercent->setText(szPercentage);

    // Adjust bubble-help according to AutoMint settings
    QString automintHelp = tr("Current percentage of zSTMP.\nIf AutoMint is enabled this percentage will settle around the configured AutoMint percentage (default = 10%).\n");
    ...
    if (fEnableZeromint) {
        ...
        automintHelp += tr("To disable AutoMint add 'enablezeromint=0' in stomp.conf.");
    }
    else {
        automintHelp += tr("AutoMint is currently disabled.\nTo enable AutoMint change 'enablezeromint=0' to 'enablezeromint=1' in stomp.conf");
    }
    ...
    // STMP Available
    bool showSTMPAvailable = settingShowAllBalances || stmpAvailableBalance != nTotalBalance;
    bool showWatchOnlySTMPAvailable = showSTMPAvailable || nAvailableWatchBalance != nTotalWatchBalance;
    ui->labelBalanceText->setVisible(showSTMPAvailable || showWatchOnlySTMPAvailable);
    ui->labelBalance->setVisible(showSTMPAvailable || showWatchOnlySTMPAvailable);
    ui->labelWatchAvailable->setVisible(showWatchOnlySTMPAvailable && showWatchOnly);

    // STMP Pending
    bool showSTMPPending = settingShowAllBalances || unconfirmedBalance != 0;
    bool showWatchOnlySTMPPending = showSTMPPending || watchUnconfBalance != 0;
    ui->labelPendingText->setVisible(showSTMPPending || showWatchOnlySTMPPending);
    ui->labelUnconfirmed->setVisible(showSTMPPending || showWatchOnlySTMPPending);
    ui->labelWatchPending->setVisible(showWatchOnlySTMPPending && showWatchOnly);

    // STMP Immature
    bool showSTMPImmature = settingShowAllBalances || immatureBalance != 0;
    bool showWatchOnlyImmature = showSTMPImmature || watchImmatureBalance != 0;
    ui->labelImmatureText->setVisible(showSTMPImmature || showWatchOnlyImmature);
    ui->labelImmature->setVisible(showSTMPImmature || showWatchOnlyImmature); // for symmetry reasons also show immature label when the watch-only one is shown
    ui->labelWatchImmature->setVisible(showWatchOnlyImmature && showWatchOnly); // show watch-only immature balance

    // STMP Locked
    bool showSTMPLocked = settingShowAllBalances || nLockedBalance != 0;
    bool showWatchOnlySTMPLocked = showSTMPLocked || nWatchOnlyLockedBalance != 0;
    ui->labelLockedBalanceText->setVisible(showSTMPLocked || showWatchOnlySTMPLocked);
    ui->labelLockedBalance->setVisible(showSTMPLocked || showWatchOnlySTMPLocked);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("STMP")
    updateDisplayUnit();
