...
#include "ui_zstmpcontroldialog.h"
...
std::set<std::string> ZStmpControlDialog::setSelectedMints;
std::set<CMintMeta> ZStmpControlDialog::setMints;

bool CZStmpControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZStmpControlDialog::COLUMN_DENOMINATION || column == ZStmpControlDialog::COLUMN_VERSION || column == ZStmpControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZStmpControlDialog::ZStmpControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZStmpControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZStmpControlDialog::~ZStmpControlDialog()
{
    delete ui;
}

void ZStmpControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZStmpControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZStmpControlWidgetItem* itemDenom(new CZStmpControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZStmpControlWidgetItem *itemMint = new CZStmpControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zstmpTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zstmpTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zSTMP.";
            else if (!mint.isSeedCorrect)
                strReason = "The zSTMP seed used to mint this zSTMP is not the same as currently hold in the wallet";
            else
...
void ZStmpControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZStmpControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZStmp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZStmpControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZStmpControlDialog::GetSelectedMints()
{
...
void ZStmpControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
