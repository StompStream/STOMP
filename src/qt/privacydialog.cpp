                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zSTMP ought to be enough for anybody." - Bill Gates, 2017
    ui->zSTMPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzSTMPSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zSTMPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzSTMP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSTMP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zSTMP, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zSTMP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zSTMP in ") +
...
void PrivacyDialog::on_pushButtonSpendzSTMP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zSTMP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zSTMP, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zSTMP
        sendzSTMP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zSTMP
    sendzSTMP();
}

void PrivacyDialog::on_pushButtonZStmpControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZStmpControlDialog* zStmpControl = new ZStmpControlDialog(this);
    zStmpControl->setModel(walletModel);
    zStmpControl->exec();
}

void PrivacyDialog::setZStmpControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzStmpSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzSTMP()
{
    ...
    double dAmount = ui->zSTMPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zSTMP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " STMP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zSTMP</b>";
    ...
    // use mints from zSTMP selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZStmpControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZStmpControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zSTMP was spent successfully update the addressbook with the label
    ...
    // Clear zstmp selector in case it was used
    ZStmpControlDialog::setSelectedMints.clear();
    ui->labelzStmpSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zSTMP Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Stmp, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zSTMP Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zstmpTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSTMP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zSTMP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zSTMP "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zSTMP "));
    ui->labelzSTMPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSTMP </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zSTMP </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zSTMP </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzSTMPSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzSTMP->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zSTMP
        ui->pushButtonMintzSTMP->setEnabled(false);
        ui->pushButtonMintzSTMP->setToolTip(tr("zSTMP is currently disabled due to maintenance."));

        // Spend zSTMP
        ui->pushButtonSpendzSTMP->setEnabled(false);
        ui->pushButtonSpendzSTMP->setToolTip(tr("zSTMP is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zSTMP
        ui->pushButtonMintzSTMP->setEnabled(true);
        ui->pushButtonMintzSTMP->setToolTip(tr("PrivacyDialog", "Enter an amount of STMP to convert to zSTMP", 0));

        // Spend zSTMP
        ui->pushButtonSpendzSTMP->setEnabled(true);
        ui->pushButtonSpendzSTMP->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
